...
/**
 * Gera CSV em formato chave/valor pra abrir direto no Excel. Linhas vazias
 * separam seções. Aspas duplas escapam o nome do evento (que pode ter vírgula).
 */
...
function downloadCsv(d: FinancialClosing) {
  // BOM (﻿) garante que Excel abra com UTF-8 e acentos certos.
...
export function FinancialClosingPanel({ eventId, source }: { eventId: string; source: 'list' | 'armageddon' }) {
    ...
    return (
      <div className="p-8 text-center text-[13px] text-[color:var(--text-3)]">
        <button className="btn btn-ghost" onClick={() => void load()}>
          <I.RotateCcw size={13}/> Tentar de novo
        </button>
      </div>
    );
  }
  ...
  return (
    <div className="p-5 space-y-5 financial-closing-print-root">
      {/* @media print: esconde tudo menos esse painel e os controles dele */}
      <style>{`
        @media print {
      ...
      <SectionTitle
        ...
        sub={
          <>
            Resumo agregado de apostas, ganhos, perdas e fluxo de caixa do evento.{' '}
            Janela analisada: <strong>{new Date(data.window.start).toLocaleString('pt-BR')}</strong> →{' '}
            <strong>{new Date(data.window.end).toLocaleString('pt-BR')}</strong>.{' '}
            <em>{data.window.note}</em>
          </>
        }
      />
      ...
      <div className="flex justify-end gap-2 no-print">
        ...
        <button className="btn btn-ghost" onClick={() => void load()} disabled={loading}>
          <I.RotateCcw size={13}/> {loading ? 'Recalculando…' : 'Atualizar'}
        </button>
      </div>
