...
export default function ApostasPage() {
  ...
  async function loadMyBets() {
    ...
    if (!token) return;
    try {
      const response = await fetch(`${apiUrl}/auth/my-bets`, { headers: { Authorization: `Bearer ${token}` }, cache: 'no-store' });
      if (!response.ok) return;
  ...
  async function placeBet() {
    ...
    const token = getAuthToken();
    if (!token) { setMessage('Faça login para apostar.'); return; }
  ...
  async function placeMultiRunnerBet(oddId: string) {
    ...
    setPlacingBet(true); setMessage('');
    try {
      const response = await fetch(`${apiUrl}/market/multi-runner/bet`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
