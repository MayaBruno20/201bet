...
/**
 * Hub público do Leva Tudo.
 * - Live do YouTube embeddada (event.streamUrl).
 * - Hero cinematográfico (tom AZUL) + broadcast bar ao vivo.
 * - Chaveamento de 2 chaves de 32 (mata-mata) que mostra SÓ os embates já
 *   formados (ambos os pilotos definidos) ou já auditados — sem slots vazios das
 *   rodadas futuras. A Grande Final e a Disputa de 3º lugar (2º sorteio) aparecem
 *   conforme as chaves vão sendo decididas.
 * A aposta reaproveita o fluxo existente via snapshot ao vivo (modal).
 */
...
import { LevaTudoChampionShowcase } from '@/components/apostas/leva-tudo-champion-showcase';
import { CheckeredFlagIcon, CrownIcon, BracketIcon, TrophyIcon } from '@/components/apostas/armageddon-icons';
...
type LevaEvent = {
  ...
  scheduledAt: string;
  endsAt?: string | null;
  matchups: LevaMatchup[];
};

/** Só embates já formados (2 pilotos) ou já decididos entram na visão pública. */
const hasContent = (m: LevaMatchup) => (!!m.leftDriverName && !!m.rightDriverName) || !!m.winnerSide;
...
/** Sub-rótulo da rodada. Chaves de 32 (mata-mata): 16 avos → 8 avos → Quartas →
 *  Semifinal → Final. O 2º sorteio traz a Grande Final e a Disputa de 3º lugar. */
function roundLabel(m: LevaMatchup): string {
  if (m.isThirdPlace) return 'Disputa de 3º lugar';
  if (m.isFinal) return 'Grande Final';
  const map: Record<number, string> = { 1: '16 avos', 2: '8 avos', 3: 'Quartas', 4: 'Semifinal', 5: 'Final' };
...
export default function LevaTudoHubPage() {
  const [event, setEvent] = useState<LevaEvent | null>(null);
  ...
  const groups = useMemo(() => {
    const byKey = new Map<string, LevaMatchup[]>();
    for (const m of event?.matchups ?? []) {
      if (!hasContent(m)) continue; // esconde slots vazios das rodadas futuras
      const key = m.stage === 'SECOND_DRAW'
        ? (m.isFinal ? 'Grande Final' : m.isThirdPlace ? 'Disputa de 3º lugar' : 'Fase final')
        : m.bracketKey ? `Chave ${m.bracketKey}` : 'Embates';
      (byKey.get(key) ?? byKey.set(key, []).get(key)!).push(m);
    }
    // Ordena as chaves em ordem alfabética (Chave A, B); a Grande Final e a Disputa
    // de 3º lugar vêm depois. Sem isto os grupos seguiam a ordem de inserção dos
    // dados — no mobile (1 coluna) ficava fora de ordem.
    ...
    return Array.from(byKey.entries())
      .map(([label, ms]) => {
        const byRound = new Map<number, LevaMatchup[]>();
        ...
        <>
          ...
          <section className="relative overflow-hidden border-b border-white/5">
            ...
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
              <h1 className="text-4xl sm:text-6xl font-extrabold tracking-tight">
                <span className="bg-gradient-to-br from-[#dff3ff] via-[#7cd0ff] to-[#4cc4ff] bg-clip-text text-transparent">{event.name}</span>
              </h1>
              <p className="mt-3 max-w-xl text-sm sm:text-base text-white/55">
                O torneio Leva Tudo: duas chaves de 32 pilotos em mata-mata até a Grande Final. Assista à transmissão e aposte em tempo real — tudo num lugar só.
              </p>
            </div>
          </section>
          ...
          {embed && (
            <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pt-8">
              <div className="overflow-hidden rounded-3xl border border-white/10 bg-black shadow-[0_0_60px_-15px_rgba(76,196,255,0.25)]">
                <div className="relative w-full" style={{ aspectRatio: '16 / 9' }}>
                  <iframe
                    src={embed}
                    title="Transmissão ao vivo do Leva Tudo"
          ...
          <div className="sticky z-30 border-b border-white/10 bg-[#0b0e18]/95 backdrop-blur-md" style={{ top: 'var(--app-shell-h, 64px)' }}>
            <div className="mx-auto flex max-w-7xl items-center gap-2 overflow-x-auto px-4 sm:px-6 lg:px-8 py-2.5 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
              ...
              <a href="#ao-vivo" className="inline-flex shrink-0 items-center gap-1.5 rounded-full bg-[linear-gradient(180deg,#7cd0ff,#4cc4ff)] px-4 py-1.5 text-[12.5px] font-bold text-[#04121c] shadow-[0_8px_22px_-10px_rgba(76,196,255,0.7)]">
          ...
          <section id="ao-vivo" className="pt-6 scroll-mt-28">
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
              <div className="flex items-center gap-2">
                <span className="h-5 w-1 rounded-full bg-[#4cc4ff]" />
                <h2 className="text-lg font-bold">Passadas ao vivo</h2>
              </div>
              <p className="mt-1 text-[13px] text-white/45">O coração do Leva Tudo — escolha os pilotos e monte seu bilhete (multi-aposta). Cotação dinâmica.</p>
            </div>
            <BettingExperience lockedEventId={event.eventId ?? undefined} hideHeader hideFeatured passadasOnly hideMyBets />
          </section>

          {/* ── MULTI-MERCADOS DO CAMPEONATO (Campeão / 2º / 3º) ── */}
          {event.eventId && <LevaTudoChampionShowcase eventId={event.eventId} />}
...
function MatchupRow({ m, onApostar }: { m: LevaMatchup; onApostar: (duelId: string) => void }) {
  const settled = !!m.winnerSide;
  return (
    <div className="flex items-center gap-2 rounded-lg px-2 py-1.5 text-[12.5px]"
      style={{ background: settled ? 'rgba(16,185,129,0.06)' : m.marketOpen ? 'rgba(76,196,255,0.06)' : 'transparent' }}>
      ...
      {m.marketOpen && !settled && m.duelId && (
        <button type="button" onClick={() => onApostar(m.duelId!)} className="shrink-0 rounded-md bg-[#4cc4ff]/15 px-2 py-0.5 text-[10px] font-bold text-[#4cc4ff] transition hover:bg-[#4cc4ff]/25">apostar</button>
      )}
    </div>
  );
}
...
function NoEventState() {
  return (
    <div className="mx-auto max-w-2xl px-4 py-24 text-center">
      <div className="mx-auto mb-4 grid h-16 w-16 place-items-center rounded-2xl border border-[#4cc4ff]/25 bg-[#4cc4ff]/10 text-[#4cc4ff]">
        <CheckeredFlagIcon size={30} />
      </div>
      <h1 className="text-2xl font-bold">Nenhum Leva Tudo ao vivo agora</h1>
      <p className="mt-2 text-white/50">O torneio Leva Tudo volta em breve. Enquanto isso, há outras corridas rolando.</p>
