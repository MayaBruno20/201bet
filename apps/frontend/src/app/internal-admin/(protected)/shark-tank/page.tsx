...
import { SharkTankDetail } from '@admin/components/admin/shark-tank-detail';
...
export default function SharkTankPage() {
  const [events, setEvents] = React.useState<SharkEvent[]>([]);
  const [loading, setLoading] = React.useState(true);
  const [selected, setSelected] = React.useState<SharkEvent | null>(null);
  const [createOpen, setCreateOpen] = React.useState(false);
  const [editEvent, setEditEvent] = React.useState<SharkEvent | null>(null);
  ...
  const load = React.useCallback(async () => {
    setLoading(true);
    try {
      const list = await api.get<SharkEvent[]>(ENDPOINTS.SHARK_TANK.list);
      setEvents(list);
  ...
  const create = async () => {
    ...
    setBusy(true);
    try {
      const created = await api.post<SharkEvent>(ENDPOINTS.SHARK_TANK.create, {
        name: form.name.trim(),
      ...
      push({ title: 'Shark Tank criado', body: form.name, tone: 'emerald' });
      setForm({ ...EMPTY_FORM });
      setCreateOpen(false);
      await load();
      setSelected(created);
  ...
  const toggleFeatured = async (e: SharkEvent) => {
    try {
      await api.patch(ENDPOINTS.SHARK_TANK.update(e.id), { featured: !e.featured });
  ...
  const saveEdit = async () => {
    ...
    setBusy(true);
    try {
      const updated = await api.patch<SharkEvent>(ENDPOINTS.SHARK_TANK.update(editEvent.id), {
        name: form.name.trim(),
  ...
  const cancelEvent = async (e: SharkEvent) => {
    const ok = await confirm({
      title: 'Cancelar Shark Tank?',
    ...
    if (!ok) return;
    try {
      await api.del(ENDPOINTS.SHARK_TANK.delete(e.id));
  ...
  return (
    <Page eyebrow="Operação · Eventos" title="Shark Tank"
      sub="4 chaves (A–D) de 8 pilotos → Fase Final: finalistas x Top 20 da Lista."
      actions={<>
        ...
        <button className="btn btn-primary focusable" onClick={() => { setForm({ ...EMPTY_FORM }); setCreateOpen(true); }}>
          <I.Plus size={15}/> Novo Shark Tank
        </button>
      </>}>
      <div className="grid grid-cols-12 gap-5">
        <div className="col-span-12 lg:col-span-4">
          <Card className="p-0 overflow-hidden">
            ...
            {!loading && events.length === 0 && <div className="p-6 text-center text-[12.5px] text-[color:var(--text-3)]">Nenhum Shark Tank ainda.</div>}
          ...
          {selected ? (
            <div className="space-y-5">
              <Card className="p-4 sm:p-5">
                <div className="text-[11px] font-semibold tracking-[0.14em] uppercase text-[color:var(--text-3)] mb-1">Shark Tank</div>
              ...
              <SharkTankDetail key={selected.id} eventId={selected.id} onChanged={load}/>
            </div>
          ) : (
            <Card className="p-8 sm:p-16 text-center">
              ...
              <div className="font-display text-[16px] font-semibold mt-3">Selecione um Shark Tank</div>
              <div className="text-[12.5px] text-[color:var(--text-3)] mt-1">Ou clique em "Novo Shark Tank".</div>
            </Card>
          )}
        </div>
      </div>
          ...
          <div className="surface-elev p-4 sm:p-6 w-full max-w-lg max-h-[90dvh] overflow-y-auto">
            <div className="flex items-start gap-3 mb-4">
              ...
              <div className="flex-1">
                <div className="font-display text-[18px] font-bold">{editEvent ? 'Editar Shark Tank' : 'Novo Shark Tank'}</div>
                <div className="text-[12px] text-[color:var(--text-3)]">4 chaves de 8 → Fase Final contra o Top 20 da Lista.</div>
              </div>
            </div>
