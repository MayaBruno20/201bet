...
/**
 * Máquina de apostas reutilizável (board + cotações WS + bilhete multi-bet).
 * Usada na página /apostas (sem trava) e no hub do Armageddon (travada no evento,
 * sem seletor de evento / sem header / sem faixa de embates personalizados).
 */
export function BettingExperience({
  ...
  async function loadBoardAndDefaults() {
    ...
    try {
      ...
      if (lockedEventId) {
        // Travado num evento (ex.: Armageddon): seleciona ele e ignora deep-link/primeiro.
        const ev = boardData.events.find((e) => e.id === lockedEventId);
        if (ev) {
      ...
      } else if (targetEvent && wantedDuelId) {
      ...
      <div className='mx-auto max-w-7xl px-3 pt-4 sm:px-6 sm:pt-6 lg:px-8'>
        ...
        {loading ? (
          ...
          <>
            ...
            {/* Event selector — escondido quando travado num evento (Armageddon) */}
            {!lockedEventId && (
            <div className='mt-5'>
              <EventSelector
